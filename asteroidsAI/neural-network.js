...
class NeuralNetwork{
	...
	feedForward(inputArray){
			//convert the input array into a one dimensional matrix
			...
			//find the out put values and apply the activation function
	...
	train(inputArray, targetArray){
		...
		let outputDeriv = Matrix.map(outputs, x => sigmoid(x, true));
		let outputDeltas = Matrix.multiply(outputErrors, outputDeriv);
...
class Matrix{
	...
	constructor(rows, cols, data = []){
		...
		//initialize with 0 if no data is provided
		...
		if(data == null || data.length == 0 ){
			...
			if(data.length != rows || data[0].length != cols){
				throw new Error("Incorrect data dimension");
			}
		}
	}
	set rows(rows){
	...
	static dot(m0,m1){
		if(m0.cols != m1.rows){
			...
			throw new Error("The two matrices provided are not dot compatible");
		}
	...
	// convert input array into one-rowed matrix
	...
	//find the trans pose for the matrix
	static transpose(m0){
